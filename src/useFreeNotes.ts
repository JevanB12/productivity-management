...
import {
  ...
  normalizeFreeNotes,
  saveFreeNotes,
} from './lib/freeNotesSync'
import { isSupabaseConfigured } from './lib/supabase'
import type { FreeNote } from './types'

export type FreeNotesSyncStatus =
  | 'loading'
  | 'synced'
  | 'syncing'
  | 'error'
...
function notesKey(userId: string) {
  return `study-calendar-free-notes-${userId}`
}

function notesUpdatedKey(userId: string) {
  return `study-calendar-free-notes-updated-at-${userId}`
}

function loadLocal(userId: string): FreeNote[] {
  try {
    const raw = localStorage.getItem(notesKey(userId))
    if (!raw) return []
    return normalizeFreeNotes(JSON.parse(raw))
  } catch {
    return []
  }
}

function saveLocal(userId: string, items: FreeNote[]) {
...
function isValidDraft(draft: FreeNoteDraft): boolean {
  return draft.title.trim().length > 0 || draft.body.trim().length > 0
}

function sortNotes(items: FreeNote[]): FreeNote[] {
  return [...items].sort(
    (a, b) => Date.parse(b.updatedAt) - Date.parse(a.updatedAt),
  )
}

export function useFreeNotes(userId: string) {
  const [items, setItems] = useState<FreeNote[]>([])
  const [syncStatus, setSyncStatus] = useState<FreeNotesSyncStatus>(() =>
  ...
  useEffect(() => {
    ...
    async function hydrate() {
      ...
      try {
        const cloud = await fetchFreeNotes(userId)
        if (cancelled) return
        ...
        if (cloudHasData && (!localHasData || cloudTime >= localTime)) {
          ...
          const updatedAt = await saveFreeNotes(userId, local)
          if (cancelled) return
  ...
  useEffect(() => {
    ...
    saveTimer.current = window.setTimeout(() => {
      void saveFreeNotes(userId, items)
        .then((cloudUpdatedAt) => {
  ...
  const addNote = useCallback((draft: FreeNoteDraft) => {
  ...
  const updateNote = useCallback((id: string, draft: FreeNoteDraft) => {
    if (!isValidDraft(draft)) return false
    const now = new Date().toISOString()
    const title = draft.title.trim() || 'Untitled'
    setItems((prev) =>
